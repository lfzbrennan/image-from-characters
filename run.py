...
def edge_image(image_path):
	image = cv2.imread(image_path)
	image = cv2.resize(image, (480, 320))
	tensorInput = torch.FloatTensor(image[:, :, ::-1].transpose(2, 0, 1).astype(numpy.float32) * (1.0 / 255.0))

	tensorOutput = estimate(tensorInput)

	return (tensorOutput.clamp(0.0, 1.0).numpy().transpose(1, 2, 0)[:, :, 0] * 255.0).astype(numpy.uint8)
# end
